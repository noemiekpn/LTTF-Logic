...
attlink(X):- not(link(_)),setSafe(X),assert(link(X)),assert(visited(X)),allAdjacent(X,A,A,C,C),setPending(A),setPending(C).
attlink(X):- not(link(_)),setSafe(X),assert(link(X)),assert(visited(X)),allAdjacent(X,A,A,C,D),setPending(A),setPending(C),setPending(D).
attlink(X):- not(link(_)),setSafe(X),assert(link(X)),assert(visited(X)),allAdjacent(X,A,B,C,D),setPending(A),setPending(B),setPending(C),setPending(D).
...
best_action(attackMonster(X)):-monster(X),link(Y), adjacent(X,Y),energy(E),E>10.
