...
void GUI_LoadImages() {
	if(initAddons) {
		images[GUI_Title] = al_load_bitmap("main_title.png");
		images[GUI_Tiles] = al_load_bitmap("lost_woods_tiles.png");
		images[GUI_Link] = al_load_bitmap("link_DURL.png");
		images[GUI_Monster] = al_load_bitmap("hoarder.png");
		images[GUI_Rupee] = al_load_bitmap("rupee.png");
		images[GUI_Heart]= al_load_bitmap("heart.png");
		images[GUI_Swords]= al_load_bitmap("swords.png");
		images[GUI_Pendants] = al_load_bitmap("pendants.png");

		// Mask out background
		al_convert_mask_to_alpha(images[GUI_Link], al_map_rgb(0, 128, 128));			// Cyan
		al_convert_mask_to_alpha(images[GUI_Monster], al_map_rgb(153, 153, 102));		// Beige
		al_convert_mask_to_alpha(images[GUI_Pendants], al_map_rgb(153, 153, 102));
		al_convert_mask_to_alpha(images[GUI_Rupee], al_map_rgb(153, 153, 102));
		al_convert_mask_to_alpha(images[GUI_Heart], al_map_rgb(255, 0, 255));			// Magenta
		al_convert_mask_to_alpha(images[GUI_Swords], al_map_rgb(255, 0, 255));			
	} else {
...
void GUI_DrawMainTitle() {
	if(initAddons) {
		...
		// Main title logo
		al_draw_bitmap(images[GUI_Title], 228, 214, 0);
...
void GUI_DrawMap(Map *map, ALLEGRO_BITMAP *image) {
	...
	char *visited = MAP_GetMapVisited(map);
	...
	if(initAddons) {
		for(int column = 0; column < size; column++) {
			...
			// All unknown positions will be covered with "fog"
			if(!visited[column]) {
