...
int main(void) {
	...
	fprintf(gameLog, "# Assert Link's start point:\tattlink(point(38,21)).\n");
	...
	while(!endGame) {
		...
		//------------------------------------------------------------
			...
			if(!link.alive || link.realSword) {
				...
				fprintf(gameLog, "# Query Link's point:\tcall(link, X).\n");
				...
				fprintf(gameLog, "# Query best action:\tcall(best_action, X).\n");
				...
				if(!strcmp((char*)av.name(), "game_over_lost")) {
					...
					if(MAP_SearchPositionObject(lostWoods, MAP_GetMapWidth(lostWoods) * link_y + link_x, MAP_ObjRealSword) != -1) {
						...
						fprintf(gameLog, "\t# Assert real sword point:\trealSword(point(%d,%d)).", link_x, link_y);
